...
export default function App() {
  ...
  useEffect(() => {
    if (sprints.length > 0 && !activeSprintId) setActiveSprintId(sprints[0].id);
  }, [sprints, activeSprintId]);

  // Reset sprint selection when board changes
  useEffect(() => { setActiveSprintId(null); }, [activeBoardId]);
          ...
          {sprints.length > 0 && (
            <SprintSelector
              sprints={sprints.map(s => ({
                ...s,
                tasks: tickets.filter(t => {
                  // For sprint selector, show tickets matching this sprint
                  // But we only have current sprint's tickets loaded
                  return true;
                }),
              }))}
